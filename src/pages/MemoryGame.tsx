...
export default function MemoryGame(){
  ...
  function handleSelectCard(i:number, imageId:number){
    console.log(i, imageId);
    console.log("CORRECT: " + correctlyGot);
    if(prev.value === null){
      if(correctlyGot.value.includes(imageId)){
        prev.value = null;
        prevId.value = null;
        currentId.value = null;
      }
  ...
  return(
    <div>
      ...
      <form onSubmit={handleChangeGame}>
        ...
        <button type="submit">Change Number of Matches</button>
      ...
      images.value.map((image, i)=>{
        return <div key={i} className="cards" height='300px' style={correctlyGot.value.includes(image.id) || prevId.value === i || currentId.value === i? `background-image: url(${image.url})`: "background-image: url('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQjNHlCAhNnlQ4pCIluYMG_9EPMnBMPrVovkg&s')" }onClick={()=> {handleSelectCard(i, image.id)
        }}></div>
        })
        }
      </div>
